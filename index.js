...
function customAssign(...objects) {
  if (objects.length < 2) {
    ...
    let newArr = Object.assign({}, ...objects);
    for (let value in newArr) {
      if (typeof newArr[value] === "number") {
        newArr[value] = String(newArr[value]);
      }
    }
    return newArr;
  }
...
function customObjectFromEntries(entries) {
  if (!Array.isArray(entries)) {
    ...
    const resultMap = entries.map(([key, value]) => {
...
function extendAndFreezeObject(obj1, obj2) {
  if (typeof obj1 !== "object" || typeof obj1 !== "object") {
    ...
    let newArr = Object.freeze(Object.assign(obj1, obj2));
    if (Object.isFrozen(newArr)) {
      return newArr;
    } else {
      return `Error`;
    }
...
function filterObjectsByKey(arr, key) {
  if (!Array.isArray) {
    ...
    const filtered = arr.filter((value) => value.hasOwnProperty(key));
    return filtered;
  }
...
function getObjectValuesSum(obj) {
  if (typeof obj !== "object") {
    ...
    const sum = valuesArray.reduce((num, value) => {
      return num + value;
    });
    return sum;
  }
